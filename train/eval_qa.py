...
def evaluate(args, model, tokenizer, eval_dataset, is_train=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():  # TODO: fix all indentation
           ...
           for k in range(len(choice_lists)):
                ...
                ins_answers = [f"<pad> <extra_id_0> {choice_lists[k][j]}" for j in range(len(batch["input_ids"]))]
                decoder_input_ids = tokenizer(ins_answers, add_special_tokens=False, padding='longest', return_tensors="pt").input_ids.to(args.device)
           ...
           max_ind = torch.argmax(torch.tensor(choice_probs), dim=0)
           for i in range(len(batch["input_ids"])):
               all_preds.append(choice_lists[max_ind[i]][i])
...
def main():
    args = get_args()
    wandb.init(project="oLMpics", entity="frostbyte", group="T5-QA", \
