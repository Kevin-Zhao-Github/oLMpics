...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, kz_layer=-1, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.kz_layer = kz_layer
    ...
    def forward(
        ...
        if output_attentions:
            # if output_values:
            #outputs = outputs + (attn_weights, value_states)  # TODO
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, kz_layer, has_relative_attention_bias=False):
        super().__init__()
        self.kz_layer = kz_layer
        self.SelfAttention = T5Attention(config, kz_layer, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, kz_layer, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, kz_layer, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, -1 if config.is_decoder else i, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
