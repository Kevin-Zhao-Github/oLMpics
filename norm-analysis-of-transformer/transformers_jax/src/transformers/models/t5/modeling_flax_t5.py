...
class FlaxT5LayerFF(nn.Module):
    ...
    def setup(self):
        ...
        self.layer_norm = nn.LayerNorm(epsilon=self.config.layer_norm_epsilon, dtype=self.dtype) \
            if hasattr(self.config, "layer_norm") and self.config.layer_norm.lower().startswith("vanilla") \
            else FlaxT5LayerNorm(self.config.d_model, eps=self.config.layer_norm_epsilon, dtype=self.dtype)
        self.dropout = nn.Dropout(self.config.dropout_rate)

    def __call__(self, hidden_states, deterministic=True):
        post_norm = hasattr(self.config, "norm_position") and self.config.norm_position.lower().startswith("post")
        if not post_norm:
            hidden_states = self.layer_norm(hidden_states)

        forwarded_states = self.DenseReluDense(hidden_states, deterministic=deterministic)
...
class FlaxT5LayerSelfAttention(nn.Module):
    ...
    def __call__(
        ...
        if post_norm:  # Layer Norm after attention and residual connection
            hidden_states = self.layer_norm(hidden_states)
...
class FlaxT5Stack(nn.Module):
    ...
    def __call__(
        ...
        if not (hasattr(self.config, "norm_position") and self.config.norm_position.lower().startswith("post")):  # if Pre-Norm
