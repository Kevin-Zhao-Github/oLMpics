...
class DataTrainingArguments:
    ...
    dataset_pickle_path: Optional[str] = field(
        default=None, metadata={
            "help": "Path to pickle file with processed data, using this argument means datasets library is not used."}
    )
    ...
    def __post_init__(self):
        if self.dataset_pickle_path is None and self.dataset_name is None and self.train_file is None \
                and self.validation_file is None:
            raise ValueError(
                "Need either a dataset name or a training/validation file or path to processed pickled data.")
        else:
